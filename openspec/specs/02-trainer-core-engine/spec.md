...
- `startCoreSession` MUST generar un `sessionId` con el formato `` `session_${Date.now()}_${Math.random().toString(36).slice(2, 7)}` ``.
- `generateQuestionToken(prefix = 'token')` MUST generar un token con el formato `` `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 7)}` ``.
