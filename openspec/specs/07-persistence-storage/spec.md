...
El sistema MUST (DEBE) abrir exactamente una base de datos IndexedDB con nombre canónico `MusicalEarTrainerDB` (`DB_NAME`) y versión canónica `5` (`DB_VERSION`).

- `DB_VERSION` MUST ser `5`, y `getVersion()` MUST devolver `db.version` tras la apertura, o `0` mientras la base no haya sido abierta.
...
- En `onupgradeneeded` la creación MUST ser idempotente: un store solo se crea si `db.objectStoreNames` no lo contiene; el índice `targetMode` en `sessions` y los índices `sessionId` y `expectedNote` en `exercise_answers` solo se crean si no existen.
...
- **THEN** `getVersion()` devuelve `5` y `getSummary()` devuelve un resumen con `totalSessions === 0`
...
- **THEN** `backup.sessions.length === 1`, `backup.answers.length === 1` y `backup.version === 5`
