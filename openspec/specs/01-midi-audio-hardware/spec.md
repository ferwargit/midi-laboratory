...
El Pánico MIDI MUST (DEBE) silenciar de forma determinista todo el hardware, tanto notas lógicas como estados de pedal, sin excepciones.

La función `sendAllNotesOff(channel = 1)` MUST:
...
4. Si la salida está disponible, emitir:
   ...
   - `Note Off` explícito para las 88 notas físicas del piano (21 a 108) — `[0x80 | (channel-1), note, 0]`
5. Si no hay salida, MUST retornar de forma segura tras limpiar el estado interno.
