...
- [x] 6.5 Al archivar el cambio, editar directamente `openspec/specs/03-practice-modalities/spec.md` (requisito *Modos de Evaluación Rítmica y Tolerancia (Modalidad 04)*) y `openspec/specs/01-midi-audio-hardware/spec.md` (requisito *Scheduler y Reloj Maestro Cuantizado (StimulusScheduler)*) con el contenido de las deltas, y releer que no quede ninguna referencia a la fórmula antigua basada en `durationBeats` del evento previo.
